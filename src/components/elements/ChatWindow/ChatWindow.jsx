...
export default function ChatWindow({ conv, onClose }) {
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            try {
                ...
                setMessages(res.data.data.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)));
            } catch (error) {
    ...
    return (
        <Card
            ...
            bodyStyle={{ padding: 0, display: "flex", flexDirection: "column", height: 400 }}
        >
            {/* Vùng hiển thị tin nhắn - Đảm bảo nó chiếm đủ không gian còn lại */}
            {/* Loại bỏ `maxHeight` cố định, `flex-1` sẽ giúp nó mở rộng */}
            <div className="flex-1 overflow-y-auto px-3 py-2" style={{ /* maxHeight: 280 (remove this) */ }}>
                {loading ? (
