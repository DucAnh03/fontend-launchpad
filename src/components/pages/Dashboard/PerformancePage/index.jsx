...
import { Card, List, Avatar, Tag, Spin, Typography, Statistic, Row, Col, Empty, Tooltip, Button } from 'antd';
import { getMyProjects, getProjectMembers } from '@/services/api/project';
import api from '@/services/api/axios';
import { CheckCircleOutlined, ClockCircleOutlined, UserOutlined, ExclamationCircleOutlined, UserDeleteOutlined } from '@ant-design/icons';
...
export default function PerformancePage() {
    ...
    async function handleRemoveMember(projectId, memberId) {
        try {
            ...
            await api.put(`/projects/${projectId}/tasks/unassign-member`, { memberId });
            // Reload lại dữ liệu
            setProjects(projects => projects.map(p =>
                p._id === projectId
                    ? {
                        ...p,
                        tasks: p.tasks.map(t => t.assigneeId?._id === memberId ? { ...t, assigneeId: null } : t),
    ...
    return (
        <div className="p-4 min-h-screen bg-gray-50">
            <Card
                title={<span className="text-2xl font-bold text-blue-700">Hiệu suất các dự án</span>}
            ...
            >
                {loading ? (
                    <div className="text-center py-10"><Spin size="large" /> Đang tải...</div>
                ) : projects.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                        {projects.map(project => {
                            ...
                            return (
                                <Card
                                    key={project._id}
                                    title={<span className="text-lg font-bold text-blue-700">{project.name}</span>}
                                    className="mb-2 shadow-lg rounded-2xl border border-gray-100 bg-white flex flex-col justify-between"
                                    headStyle={{ background: '#f5faff', borderRadius: '16px 16px 0 0', fontWeight: 700 }}
                                    bodyStyle={{ minHeight: 180, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}
                                >
                                    <Row gutter={16} className="mb-2">
                                        <Col span={12}>
                                            <Statistic
                                                title={<span className="font-medium text-gray-600">Số task hoàn thành</span>}
                                        ...
                                        <Col span={12}>
                                            <Statistic
                                                title={<span className="font-medium text-gray-600">Số task trễ deadline</span>}
                                                value={overdueTasks.length}
                                                valueStyle={{ color: overdueTasks.length > 0 ? '#ff4d4f' : '#222' }}
                                                prefix={<ExclamationCircleOutlined style={{ color: '#ff4d4f' }} />}
                                            />
                                        </Col>
                                    </Row>
                                    <div className="mb-2">
                                        <Typography.Text strong className="block mb-1">Thành viên:</Typography.Text>
                                            ...
                                            {project.members.length === 0 ? (
                                                ...
                                                <Tooltip title={member.userId.username} key={member.userId._id}>
                                                    <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-blue-50 border border-blue-100 text-blue-700 text-sm font-medium">
                                                        ...
                                                        {member.userId.name}
                                                        <Tag color={member.role === 'leader' ? 'green' : 'blue'} className="ml-1" style={{ marginLeft: 4 }}>{member.role === 'leader' ? 'Leader' : 'Member'}</Tag>
                                                    </span>
                                                </Tooltip>
                                            ))}
                                        </div>
                                    ...
                                    {overdueTasks.length > 0 && (
                                        <div className="mt-2 p-3 rounded-xl bg-red-50 border border-red-200">
                                            <Typography.Text strong className="text-red-600 block mb-2"><ExclamationCircleOutlined /> Người bị trễ deadline:</Typography.Text>
                                            <List
                                                dataSource={overdueTasks}
                                                locale={{ emptyText: <span className="text-gray-400 italic">Không có task trễ deadline</span> }}
                                                renderItem={task => {
                                                    const member = project.members.find(m => m.userId._id === (task.assigneeId?._id));
                                                    return (
                                                        <List.Item key={task._id} className="!py-1 flex items-center justify-between">
                                                            <span className="text-sm">
                                                                <b>{task.assigneeId?.name || '(Chưa gán)'}</b> - Task: <b>{task.name}</b> <ClockCircleOutlined className="ml-1 text-red-400" /> (Hạn: {task.dueDate ? new Date(task.dueDate).toLocaleDateString() : 'N/A'})
                                                            </span>
                                                            {isLeader(project) && task.assigneeId?._id && member && member.role !== 'leader' && (
                                                                <Button
                                                                    type="link"
                                                                    icon={<UserDeleteOutlined style={{ color: '#ff4d4f' }} />}
                                                                    danger
                                                                    onClick={() => handleRemoveMember(project._id, task.assigneeId._id)}
                                                                >
                                                                    Loại thành viên
                                                                </Button>
                                                            )}
