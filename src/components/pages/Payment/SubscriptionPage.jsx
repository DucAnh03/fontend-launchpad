...
const SubscriptionPage = () => {
  ...
  useEffect(() => {
    api.get('/payment/plans')
      ...
      .catch(() => setError('Không lấy được danh sách gói'));
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-100 via-purple-50 to-pink-100 py-12 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 text-lg">Khám phá sức mạnh của Crystal Gems! 💎</p>
        </div>
          ...
          {plans.map((plan, index) => {
            ...
            return (
              <div key={plan._id} className="relative group">
                {/* Magical aura */}
                <div className={`absolute -inset-1 bg-gradient-to-r ${theme.borderGradient} rounded-2xl opacity-0 group-hover:opacity-30 blur transition-all duration-700`}></div>
                
                {/* Main card */}
                <div className={`relative bg-gradient-to-br ${theme.bgGradient} backdrop-blur-sm rounded-2xl shadow-xl border-2 border-white/50 p-8 flex flex-col items-center transition-all duration-500 hover:scale-105 hover:shadow-2xl`}>
                  ...
                  <h2 className={`text-2xl font-bold mb-3 bg-gradient-to-r ${theme.gradient} bg-clip-text text-transparent drop-shadow-sm`}>
                    {plan.planName}
                  </h2>
                  
                  {/* Price with magical styling */}
                  <div className="relative mb-4">
                    <div className={`text-4xl font-bold bg-gradient-to-r ${theme.gradient} bg-clip-text text-transparent drop-shadow-lg`}>
                      {plan.price.toLocaleString()}₫
                    </div>
                    {/* Price glow effect */}
                    <div className={`absolute inset-0 text-4xl font-bold bg-gradient-to-r ${theme.gradient} bg-clip-text text-transparent opacity-50 blur-sm`}>
                      {plan.price.toLocaleString()}₫
                    </div>
                  </div>
                  ...
                  <ul className="mb-8 text-sm text-gray-700 space-y-2 w-full">
                    {(plan.features || []).map((f, i) => (
                      <li key={i} className="flex items-start gap-3">
                        <div className={`w-2 h-2 bg-gradient-to-r ${theme.gradient} rounded-full mt-2 flex-shrink-0`}></div>
        ...
        <div className="text-center mt-12">
          <div className="inline-flex items-center gap-2 text-gray-500">
            ...
            <span className="text-sm">Được bảo vệ bởi sức mạnh Crystal Gems</span>
