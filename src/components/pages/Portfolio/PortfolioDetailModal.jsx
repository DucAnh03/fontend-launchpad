    ...
    useEffect(() => {
        ...
        const fetchItemDetails = async () => {
            try {
                ...
                setError(null);
                setItem(null);
                const portfolioData = await getPortfolioById(itemId);
                if (portfolioData) {
                    setItem(portfolioData); 
                } else {
                    setError("Không tìm thấy thông tin chi tiết của dự án này.");
                }

            } catch (err) {
                setError("Đã xảy ra lỗi không mong muốn. Vui lòng thử lại.");
                console.error("Lỗi không xác định trong fetchItemDetails:", err);
            } finally {
                setLoading(false);
            }
        };
